        ...
        <div class="col-md-6">
            <form action="/posts">
                @if (request('category') || request('author'))
                    <input type="hidden" name="{{ request('category') ? 'category' : 'author' }}"
                        value="{{ request('category') ?? request('author') }}">
                @endif
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <h5 class="card-title"><a class="text-decoration-none text-dark"
                                        href="{{ $post->slug }}">{{ $post->title }}</a></h5>
                                <p>
                                    <small class="text-muted">
                                ...
                                <a href="/posts/{{ $post->slug }}" class="btn btn-primary btn btn-primary">Read more</a>
                            </div>
                        </div>
                    </div>
                @endforeach
